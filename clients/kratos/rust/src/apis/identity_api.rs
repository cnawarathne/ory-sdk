...
/// struct for typed errors of method [`list_identity_devices`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListIdentityDevicesError {
    Status400(models::ErrorGeneric),
    Status404(models::ErrorGeneric),
    DefaultResponse(models::ErrorGeneric),
    UnknownValue(serde_json::Value),
...
/// struct for typed errors of method [`patch_identity_devices`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PatchIdentityDevicesError {
    Status400(models::ErrorGeneric),
    Status404(models::ErrorGeneric),
    Status409(models::ErrorGeneric),
    DefaultResponse(models::ErrorGeneric),
...
pub async fn list_identity_devices(configuration: &configuration::Configuration, id: &str) -> Result<Vec<models::SessionDevice>, Error<ListIdentityDevicesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;

    let uri_str = format!("{}/admin/identities/{id}/devices", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::SessionDevice&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::SessionDevice&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListIdentityDevicesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn patch_identity_devices(configuration: &configuration::Configuration, id: &str, device_id: &str) -> Result<Vec<models::SessionDevice>, Error<PatchIdentityDevicesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_device_id = device_id;

    let uri_str = format!("{}/admin/identities/{id}/devices/{device_id}", configuration.base_path, id=crate::apis::urlencode(p_id), device_id=crate::apis::urlencode(p_device_id));
    let mut req_builder = configuration.client.request(reqwest::Method::PATCH, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<PatchIdentityDevicesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
