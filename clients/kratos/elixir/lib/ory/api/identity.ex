...
defmodule Ory.Api.Identity do
  ...
  - `{:ok, [%SessionDevice{}, ...]}` on success
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec list_identity_devices(Tesla.Env.client, String.t, keyword()) :: {:ok, Ory.Model.ErrorGeneric.t} | {:ok, [Ory.Model.SessionDevice.t]} | {:error, Tesla.Env.t}
  def list_identity_devices(connection, id, _opts \\ []) do
    request =
      %{}
      |> method(:get)
      |> url("/admin/identities/#{id}/devices")
      |> Enum.into([])

    connection
    |> Connection.request(request)
    |> evaluate_response([
      {200, Ory.Model.SessionDevice},
      {400, Ory.Model.ErrorGeneric},
      {404, Ory.Model.ErrorGeneric},
      {:default, Ory.Model.ErrorGeneric}
    ])
  ...
  - `connection` (Ory.Connection): Connection to server
  - `id` (String.t): ID is the session's ID.
  - `device_id` (String.t): DeviceID is the Identity's Device's ID.
  - `opts` (keyword): Optional parameters
  ...
  - `{:error, Tesla.Env.t}` on failure
  """
  @spec patch_identity_devices(Tesla.Env.client, String.t, String.t, keyword()) :: {:ok, Ory.Model.ErrorGeneric.t} | {:ok, [Ory.Model.SessionDevice.t]} | {:error, Tesla.Env.t}
  def patch_identity_devices(connection, id, device_id, _opts \\ []) do
    request =
      %{}
      |> method(:patch)
      |> url("/admin/identities/#{id}/devices/#{device_id}")
      |> ensure_body()
      |> Enum.into([])
