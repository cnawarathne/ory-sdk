...
class SuccessfulNativeLogin(BaseModel):
    ...
    session: Session
    session_token: Optional[StrictStr] = Field(default=None, description="The Session Token  A session token is equivalent to a session cookie, but it can be sent in the HTTP Authorization Header:  Authorization: bearer ${session-token}  The session token is only issued for API flows, not for Browser flows!")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["continue_with", "session", "session_token"]
