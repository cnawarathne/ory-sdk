...
export function instanceOfRegistrationFlowState(value: any): boolean {
    for (const key in RegistrationFlowState) {
        if (Object.prototype.hasOwnProperty.call(RegistrationFlowState, key)) {
            if (RegistrationFlowState[key as keyof typeof RegistrationFlowState] === value) {
                return true;
            }
        }
    }
...
export function RegistrationFlowStateToJSONTyped(value: any, ignoreDiscriminator: boolean): RegistrationFlowState {
    return value as RegistrationFlowState;
}
